...
std::vector<double> PA10Controller::InverseKinematics(std::vector<double> end_effector_vector, std::vector<std::vector<double>> transformation_matrix, std::vector<double> cartesian_vector_0)
{
    ...
    nxp = (cos(theta1) * cos(theta2) * cos(theta3)) - (cos(theta1) * sin(theta2) * sin(theta3));
    nyp = -sin(theta1);

    txp = (cos(theta2) * cos(theta3) * sin(theta1)) - (sin(theta1) * sin(theta2) * sin(theta3));
    typ = cos(theta1);

    bxp = -sin(theta2 - theta3);
    byp = 0;
    bzp = (cos(theta2) * cos(theta3)) - (sin(theta2) * sin(theta3));
